...
def _audit_prompt(job_id, candidate_questions):
    ...
    return (
        ...
        "不检查答案；knowledge_check 固定 not_reviewed。答案或解析字段可能已由独立来源答案流程写入，"
        "它们不在单题图中，审核时完全忽略答案与解析字段，不得因其非空而判错或据此判断公式一致性。"
        "auto_pass 当且仅当 "
...
def claim_candidate_audit(database_path, private_root, job_id, runner=None):
