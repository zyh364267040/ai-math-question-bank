...
两份真实试卷已经在本地私有数据中完成闭环验证，累计 42 道正式题（这些题目和原卷不会上传到 Git）。`v0.2.0` 已实现前三步：“用户主动选择 PDF → 预览并确认 → 创建待处理任务 → 用户明确启动页面渲染 → 用户再次明确启动版面分析 → 查看单栏、双栏、三栏及题目边界候选预览”。版面分析只使用 PDF 自带文本层辅助识别主题号；无文本层时不会猜题，也不会自动启动 OCR、生成单题裁图或修改题库。下一阶段将建设边界人工调整、确认和单题裁图流程；AI/OCR 识别和候选生成仍由仓库外工具完成。
