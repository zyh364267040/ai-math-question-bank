...
- 页面渲染支持进度查看、并发防重、失败重试、崩溃恢复和原子发布，不会自动启动 OCR、切题或 AI
...
两份真实试卷已经在本地私有数据中完成闭环验证，累计 42 道正式题（这些题目和原卷不会上传到 Git）。`v0.2.0` 前两步已经实现：“用户主动选择 PDF → 预览并确认 → 创建待处理任务 → 用户明确启动 → 安全渲染页面并查看进度”。下一阶段将建设版面识别与题目边界计算；AI/OCR 识别和候选生成仍由仓库外工具完成。
